...
<table border="1" width="300px" height="50px" cellpadding="20">
  <tr><th>Serial No</th><th>Employee Name</th><th>Mail</th><th>Mobile</th><th>Department</th><th>Manager Name</th><th>FROM DATE</th><th>TO DATE</th><th>RemainingLeaves</th><th>Leave Status</th></tr>
<% 
   Connection con=null;
try{
  ...
  <td><%=rs.getString(11) %></td><td><%=rs.getString(8) %><td><%=rs.getString(9) %><td><%=rs.getInt(7) %></td><td><%=rs.getString(12) %></td><td><a href="accept.jsp?totalleave=<%=rs.getString(7) %>&requestleave=<%=rs.getString(10) %>&employeeid=<%=rs.getInt(3) %>">accept/regect</a></td>
